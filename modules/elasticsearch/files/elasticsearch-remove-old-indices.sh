...
INDICES_TEXT=$(curl -s http://localhost:9200/_status?pretty=true | perl -MJSON -e '$j=new JSON; $/=undef; $p=$j->decode(<>); print join("\n",keys %{$p->{indices}}),"\n"' | grep $GREP | sort -r)
  ...
  for index in ${INDEX[@]:$KEEP};do
    ...
    if [ -n "$index" ]; then
      if [ -z "$LOGFILE" ]; then
        ...
        echo -n `date "+[%Y-%m-%d %H:%M] "`" Deleting index: $index: " >> $LOGFILE
