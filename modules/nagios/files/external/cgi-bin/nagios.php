<?php
 ...
 $pagetitle = "Tech Platform Nagios Dashboard";
...
<a href="/nagios/cgi-bin/availreport.pl?show_log_entries=&servicegroup=HTTP&timeperiod=last24hours&assumeinitialstates=yes&assumestateretention=yes&assumestatesduringnotrunning=yes&includesoftstates=no&initialassumedhoststate=3&initialassumedservicestate=6&backtrack=4">Availability Reporting Last 1 Day</a>&nbsp
<a href="/nagios/cgi-bin/availreport.pl?show_log_entries=&servicegroup=HTTP&timeperiod=last7days&assumeinitialstates=yes&assumestateretention=yes&assumestatesduringnotrunning=yes&includesoftstates=no&initialassumedhoststate=3&initialassumedservicestate=6&backtrack=4">Availability Reporting Last 7 Days</a>&nbsp
<a href="/nagios/cgi-bin/availreport.pl?show_log_entries=&servicegroup=HTTP&timeperiod=last31days&assumeinitialstates=yes&assumestateretention=yes&assumestatesduringnotrunning=yes&includesoftstates=no&initialassumedhoststate=3&initialassumedservicestate=6&backtrack=4">Availability Reporting Last 31 Days</a>&nbsp
...
function dashdisplay($dashline,$colhost){      //function to display array data into table with colors/font etc
 ...
 if (strpos(strtoupper($dashservice),"HTTP") === false) {
  return (1);
 }
  ...
  if ($fontcolor!=""){
    ...
    if ($colhost==true){
     if ($dashack==0){
      echo("<td><font color=\"$fontcolor\">".substr($dashservice,0,strpos($dashservice," "))."</font></td>");
     }elseif ($dashack==1){
...
<a href="/nagios/cgi-bin//status.cgi?hostgroup=all&style=grid">Sysadmin Host View</a>&nbsp
<a href="/nagios/cgi-bin//status.cgi?host=all">Sysadmin Service View</a>
