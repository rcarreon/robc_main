...
if [[ ${ROLLBACK} -eq 1 ]]; then
    ...
    mv -fT ${HTDOCS}.prev ${HTDOCS} && touch ${DOCROOT} && sync
else
    ...
    rsync --delete -include=tags -avhzC "${BUILDROOT}/" ${DOCROOT}.next && mv -fT ${DOCROOT} ${DOCROOT}.prev && mv -fT ${DOCROOT}.next ${DOCROOT} && touch ${DOCROOT} && sync
