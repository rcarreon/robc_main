node 'spx2v-vb.ao.prd.lax.gnmedia.net' {
    include newrelic
    ...
    $prd_vb_2_bab_spx=decrypt('vafWaej1hncjLueAt5+//A==')
    $prd_vb_2_hfb_spx=decrypt('E4nCXLeOWP5ntYqEUf361A==')
    $prd_vb_2_sdc_spx=decrypt('6hBuvAtHdhsazUhXUUM5kQ==')
    $prd_vb_2_shh_spx=decrypt('z4+CT7ExY11Wf8AHaxR/SA==')
    $prd_vb_2_tfs_spx=decrypt('7qvigF+OobiVl9kYitFTPQ==')
    $prd_vb_2_wz_spx=decrypt('SNHWdXt2NBJztV6SlovZvA==')

    sphinx::multiple_sphinx_config 
        {"shh": sql_host => 'vip-sqlro-vb-shh.ao.prd.lax.gnmedia.net', 
        sql_user         => 'sphinx', 
        sql_pass         => "$prd_vb_2_shh_spx", 
        sql_db           => 'forums_shh', 
        port             => '3312'
        }
    ...
    common::nfsmount { "/sphinx":
        device => "nfsA-netapp1.gnmedia.net:/vol/nac1a_ao_lax_prd_spx_shared/spx2v-vb.ao.prd.lax.gnmedia.net",
    }
