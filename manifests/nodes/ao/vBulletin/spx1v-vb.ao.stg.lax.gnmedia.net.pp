node 'spx1v-vb.ao.stg.lax.gnmedia.net' {
    include newrelic
    ...
    $stg_vb_bab_spx=decrypt('vafWaej1hncjLueAt5+//A==')
    $stg_vb_hfb_spx=decrypt('E4nCXLeOWP5ntYqEUf361A==')
    $stg_vb_sdc_spx=decrypt('6hBuvAtHdhsazUhXUUM5kQ==')
    $stg_vb_shh_spx=decrypt('OxKWN3fDN5cqiTdEg8AahA==')
    $stg_vb_tfs_spx=decrypt('O8baYf+MVEG+8f0e2DzDtA==')
    $stg_vb_wz_spx=decrypt('UqXdRKZlR0Xe6yRXC9YzIw==')

    sphinx::multiple_sphinx_config 
        {"shh": sql_host => 'vip-sqlrw-vb-shh.ao.stg.lax.gnmedia.net', 
        sql_user         => 'sphinx', 
        sql_pass         => "$stg_vb_shh_spx", 
        sql_db           => 'forums_shh', 
        port             => '3312'
        }
    ...
    common::nfsmount { "/sphinx":
        device => "nfsA-netapp1.gnmedia.net:/vol/nac1a_ao_lax_stg_spx_shared/spx1v-vb.ao.stg.lax.gnmedia.net",
    }
