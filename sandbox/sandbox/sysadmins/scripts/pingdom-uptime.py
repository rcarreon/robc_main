...
def get_info(conn, siteid, fromDate):
	...
	avgUptime = '%.4f' % avgUptime
	return (avgResponseTime, avgUptime)

def get_bu(site):
	osCmd = "rt list -l -t asset \"Name = '%s'\"" % site
	a = os.popen(osCmd)
	b = a.readlines()
	if b == ['No matching results.\n']:
		return "No Business Unit"
	elif "CF.{Business Unit}:\n" in b:
		return "No Business Unit"
	for entry in b:
		if entry.strip().startswith("CF.{Business Unit}:"):
			return entry.strip().replace("CF.{Business Unit}: ", "")
	return "error %s" % b
