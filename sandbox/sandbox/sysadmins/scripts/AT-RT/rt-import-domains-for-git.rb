...
# Imports domain info from CSV files stored in GIT to RT inventory system
...
`#{GIT_BIN} clone #{GIT_PATH} /tmp/domains`
DOMAIN_FILES=`ls #{TMP_DIR}`.split.select{|f| File.extname(f) == ".csv"}
DOMAIN_FILES.each do |file|
    `ls #{TMP_DIR}/#{file}`
  if $?.success?
    import_to_rt("#{TMP_DIR}/#{file}")
  end
end
