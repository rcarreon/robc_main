...
def send_mail(content,subject,send_to):
    ...
    result = "Hi,\nSomething went wrong during parsing core1 rancid config:\n\n" + content
    pp.write(result)
    exitcode = pp.close()
    if exitcode:
...
def loadconf(filename):
    try:
        ...
        for x in L4_lines:
            ...
            vip_node = Device()
            print "VAL", virtual_address_lines
    ...
    #        vip_node._types = type_node
            ...
            if len(L7_policy) == 1:
                ...
                if len(single_policy_type_conf) == 0:
                    ...
                    if len(serverfarms) >= 1:
                        if "BOT" in serverfarms[0] and len(serverfarms) > 1:
                            # swapping
                            serverfarms[0] = serverfarms[1]
        ...
        send_mail(str(err), "WARNING: Vipvisual core1 parsing error", "sysadmins@gorillanation.com" )
        print str(err)
        sys.exit()
